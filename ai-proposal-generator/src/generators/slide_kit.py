...
def _to_emu(v):
    """좌표/크기 인자를 EMU로 자동 변환.

    - ``float`` (예: 2.5) → ``Inches(2.5)`` 로 변환 (인치로 해석)
    - ``int`` 가 1000 이하 → 인치로 해석 (예: 1, 2, 5)
    - ``int`` 가 1000 초과 → EMU 그대로 (예: ``Inches(2.5)`` 결과 = 2286000)
    - ``Emu`` / ``Inches`` / ``Pt`` 결과 → 그대로
    - ``None`` → 그대로 (호출자가 default 처리)

    이 함수 덕분에 사용자가 ``T(s, 0.6, 4.0, 7.07, 1.5, ...)`` 처럼
    인치 float 을 직접 전달해도 정상 동작 (이전엔 EMU 0 으로 해석되어 슬라이드 상단 깨짐).
    """
    if v is None:
        return v
    if isinstance(v, float):
        return Inches(v)
    if isinstance(v, int):
        # EMU 는 일반적으로 100,000+ (Inches(0.1) = 91440). 1000 이하면 인치 가정
        return Inches(v) if abs(v) < 1000 else v
    return v  # Emu/Inches/Pt 객체는 그대로
...
# 컬러 팔레트 (Warm Minimal 스타일 — 베이지/뉴트럴 톤)
C = {
    "primary":   RGBColor(45, 45, 45),       # #2D2D2D  주색 (다크 차콜)
    "secondary": RGBColor(140, 130, 115),    # #8C8273  보조색 (웜 그레이)
    "teal":      RGBColor(120, 130, 110),    # #78826E  올리브 (포인트)
    "accent":    RGBColor(180, 140, 100),    # #B48C64  따뜻한 브론즈 (강조)
    "dark":      RGBColor(45, 45, 45),       # #2D2D2D  본문 기본색
    "light":     RGBColor(240, 235, 227),    # #F0EBE3  웜 베이지 배경
    "white":     RGBColor(248, 245, 240),    # #F8F5F0  따뜻한 화이트
    "gray":      RGBColor(140, 135, 128),    # #8C8780  보조 텍스트
    "lgray":     RGBColor(215, 210, 200),    # #D7D2C8  구분선 (웜 그레이)
    "green":     RGBColor(100, 120, 90),     # #64785A  자연스러운 그린
    "orange":    RGBColor(200, 160, 100),    # #C8A064  머스타드
    "gold":      RGBColor(180, 155, 110),    # #B49B6E  소프트 골드
}
...
SZ = {
    "hero":      48,   # 표지 (절제된 크기)
    "divider":   32,   # 섹션 구분자
    "action":    18,   # Action Title (슬라이드 제목)
    "subtitle":  14,   # 부제
    "body":      12,   # 본문
    "body_sm":   10,   # 본문 소
    "caption":    9,   # 캡션
...
def new_slide(prs):
    """빈 레이아웃 슬라이드 추가 (웜 베이지 배경)"""
    s = prs.slides.add_slide(prs.slide_layouts[6])
    s.background.fill.solid()
    s.background.fill.fore_color.rgb = C["light"]
    return s
...
def R(s, l, t, w, h, f=None, lc=None, lw=1):
    ...
    Args:
        ...
        lw: 테두리 두께(pt)
    """
    l, t, w, h = _to_emu(l), _to_emu(t), _to_emu(w), _to_emu(h)
...
def SRC(s, text):
    ...
    너비는 현재 ``CW`` (콘텐츠 너비) 사용 — 포맷 변경 시 자동 조정.
    """
    T(s, ML, SH - Inches(0.4), CW, Inches(0.25),
...
def IMG_PH(s, x, y, w, h, label="이미지 영역"):
    ...
    x, y, w, h = _to_emu(x), _to_emu(y), _to_emu(w), _to_emu(h)
...
THEMES = {
    "warm_minimal": {
        "primary": (45, 45, 45), "secondary": (140, 130, 115),
        "teal": (120, 130, 110), "accent": (180, 140, 100),
        "dark": (45, 45, 45), "light": (240, 235, 227),
        "white": (248, 245, 240), "gray": (140, 135, 128),
        "lgray": (215, 210, 200), "green": (100, 120, 90),
        "orange": (200, 160, 100), "gold": (180, 155, 110),
        "card_bg": (245, 242, 237), "card_border": (225, 220, 212),
        "_meta": {"margin": 1.2, "font_hero": 48, "font_action": 18, "font_body": 12},
    },
    "classic_blue": {
        "primary": (0, 44, 95), "secondary": (0, 170, 210),
        "teal": (0, 161, 156), "accent": (230, 51, 18),
        "dark": (33, 33, 33), "light": (245, 245, 245),
        "white": (255, 255, 255), "gray": (117, 117, 117),
        "lgray": (200, 200, 200), "green": (46, 125, 50),
        "orange": (245, 166, 35), "gold": (197, 151, 62),
        "card_bg": (250, 250, 252), "card_border": (230, 232, 236),
        "_meta": {"margin": 0.8, "font_hero": 60, "font_action": 20, "font_body": 13},
    },
    "forest": {
        "primary": (35, 60, 45), "secondary": (90, 140, 100),
        "teal": (60, 120, 100), "accent": (180, 120, 60),
        "dark": (30, 40, 30), "light": (238, 242, 235),
        "white": (245, 248, 243), "gray": (110, 125, 115),
        "lgray": (200, 210, 195), "green": (60, 110, 70),
        "orange": (190, 150, 80), "gold": (160, 140, 90),
        "card_bg": (242, 245, 238), "card_border": (215, 225, 210),
        "_meta": {"margin": 1.0, "font_hero": 52, "font_action": 18, "font_body": 12},
    },
    "corporate": {
        "primary": (25, 35, 55), "secondary": (70, 100, 150),
        "teal": (50, 115, 130), "accent": (200, 80, 50),
        "dark": (30, 30, 40), "light": (242, 244, 248),
        "white": (248, 249, 252), "gray": (120, 125, 135),
        "lgray": (200, 205, 215), "green": (50, 110, 80),
        "orange": (210, 150, 60), "gold": (175, 150, 100),
        "card_bg": (245, 247, 250), "card_border": (225, 228, 235),
        "_meta": {"margin": 0.9, "font_hero": 54, "font_action": 20, "font_body": 13},
    },
    "mono_black": {
        "primary": (20, 20, 20), "secondary": (80, 80, 80),
        "teal": (100, 100, 100), "accent": (200, 60, 40),
        "dark": (15, 15, 15), "light": (245, 245, 245),
        "white": (250, 250, 250), "gray": (130, 130, 130),
        "lgray": (210, 210, 210), "green": (80, 120, 80),
        "orange": (200, 150, 80), "gold": (170, 150, 110),
        "card_bg": (248, 248, 248), "card_border": (230, 230, 230),
        "_meta": {"margin": 1.0, "font_hero": 52, "font_action": 18, "font_body": 12},
    },
    "soft_purple": {
        "primary": (60, 45, 80), "secondary": (130, 100, 160),
        "teal": (90, 130, 145), "accent": (190, 100, 130),
        "dark": (45, 35, 55), "light": (242, 238, 248),
        "white": (248, 246, 252), "gray": (125, 118, 135),
        "lgray": (210, 205, 220), "green": (90, 125, 100),
...
def apply_theme(theme_name):
    """컬러 팔레트 + 여백 + 폰트 크기 일괄 변경"""
    global ML, MR, MT_Y, CW, SZ
    ...
    C["green_light"]     = lighten(C["green"], 0.80)
    # 메타 설정
    meta = theme.get("_meta", {})
    if "margin" in meta:
        ML = Inches(meta["margin"])
        MR = Inches(meta["margin"])
        MT_Y = Inches(meta["margin"] * 0.5)
        CW = SW - ML - MR
    if "font_hero" in meta:
        SZ["hero"] = meta["font_hero"]
    if "font_action" in meta:
        SZ["action"] = meta["font_action"]
    if "font_body" in meta:
        SZ["body"] = meta["font_body"]
    return theme_name


def reset_theme():
    """테마를 기본값(warm_minimal)으로 복원"""
...
def list_themes():
    ...
    for name, theme in THEMES.items():
        ...
        print(f"  {name:20s} margin={margin}\" hero={meta.get('font_hero','?')}pt")
